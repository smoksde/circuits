...
def step_3(
    ...
    for j in range(s):
        row = []
        for i in range(n):
            b_j_i = lemma_4_1.lemma_4_1(circuit, x_list[i], c_list[j], parent_group=this_group)
            row.append(b_j_i)
        matrix.append(row)
    return matrix
...
def theorem_5_2(
    ...
    c_list, c = theorem_5_3.precompute_good_modulus_sequence(circuit, zero, one, big_n, parent_group=this_group)
