...
def one_right_shift(circuit, x_list, parent_group=None):
    ...
    for x in x_list[1:]:
        result.append(x)
    result.append(
        constant_zero(circuit, x_list[len(x_list) - 1], parent_group=ors_group)
    )
    return result
    ...
    for i in range(n):
        ...
        if shift_amount <= n:
            for k in range(shift_amount):
                shifted.append(
                    constant_zero(circuit, current[0], parent_group=nls_group)
        ...
        for l in range(n):
            next_current.append(
                mux2(circuit, amount[i], shifted[l], current[l], parent_group=nls_group)
            )
...
def n_right_shift(circuit, x_list, amount, parent_group=None):
    ...
    for i in range(n):
        ...
        if shift_amount <= n:
            ...
            for k in range(shift_amount):
                shifted.append(
                    constant_zero(circuit, current[0], parent_group=nrs_group)
                )
        else:
            for m in range(n):
                shifted.append(
        ...
        for l in range(n):
            next_current.append(
                mux2(circuit, amount[i], shifted[l], current[l], parent_group=nrs_group)
            )
        current = next_current
    return current
