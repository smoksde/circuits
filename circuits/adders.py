...
def full_adder(circuit, x, y, cin, parent_group=None):
    ...
    cout = circuit.add_node(
        "or", "FA_OR", inputs=[carry1, carry2], group_id=fa_group.id
    )
...
def carry_look_ahead_adder(circuit, x_list, y_list, cin, parent_group=None):
    ...
    for x, y in zip(x_list, y_list):
        p = circuit.add_node("xor", "XOR", inputs=[x, y], group_id=claa_group.id)
        g = circuit.add_node("and", "AND", inputs=[x, y], group_id=claa_group.id)
    ...
    def build_group_pg(start, end):
        if start == end:
            ...
            p_combined = circuit.add_node(
                "and", "AND", inputs=[p_high, p_low], group_id=claa_group.id
            )

            p_high_and_g_low = circuit.add_node(
                "and", "AND", inputs=[p_high, g_low], group_id=claa_group.id
            )
            g_combined = circuit.add_node(
                "or",
                "OR",
                inputs=[g_high, p_high_and_g_low.ports[2]],
                group_id=claa_group.id,
            )
    ...
    if n > 1:
        for i in range(n):
            if i == 0:
                p0_and_c0 = circuit.add_node(
                    "and", "AND", inputs=[propagate[0], cin], group_id=claa_group.id
                )
                c1 = circuit.add_node(
                    "or",
                    "OR",
                ...
                p_group_and_cin = circuit.add_node(
                    "and", "AND", inputs=[p_group, cin], group_id=claa_group.id
                )
        ...
        c1 = circuit.add_node(
            "or", "OR", inputs=[generate[0], p0_and_c0.ports[2]], group_id=claa_group.id
        )
        carries.append(c1.ports[2])
    ...
    for i in range(n):
        sum_out = circuit.add_node(
            "xor", "XOR", inputs=[propagate[i], carries[i]], group_id=claa_group.id
        )
