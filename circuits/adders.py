...
def carry_look_ahead_adder(circuit, x_list, y_list, cin, parent_group=None):
    ...
    def build_group_pg(start, end):
        if start == end:
            ...
            p_high_and_g_low = and_gate(circuit, [p_high, g_low], parent_group=this_group)
            g_combined = or_gate(
    ...
    if n > 1:
        for i in range(n):
            if i == 0:
                ...
                p_group_and_cin = and_gate(circuit, [p_group, cin], parent_group=this_group)
                carry_term = or_gate(circuit, [g_group, p_group_and_cin], parent_group=this_group)
                pi_and_carry = and_gate(circuit, [propagate[i], carry_term], parent_group=this_group)
                ci_plus_1 = or_gate(circuit, [generate[i], pi_and_carry], parent_group=this_group)
